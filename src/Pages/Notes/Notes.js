...
import React, { useState, useEffect, useRef, useCallback } from 'react';
import { Plus, Save, FileText, Trash2, Lock } from 'lucide-react';
import './Notes.css';
...
const NotesLock = ({ onUnlock }) => {
  ...
  const setPassword = () => {
    const pwd = prompt('Set your Notes Password:');
    if (pwd && pwd.trim() !== '') {
      localStorage.setItem('notespassword', pwd.trim());
      setHasPassword(true);
      alert('Password set successfully!');
      onUnlock();
    }
  ...
  const resetPassword = () => {
    if (window.confirm('Reset password? You will need to set a new one.')) {
      localStorage.removeItem('notespassword');
      setHasPassword(false);
      setError('');
    }
...
const Notes = () => {
  ...
  // ── On mount: load notes & check if password exists ──
  useEffect(() => {
    const savedNotes = localStorage.getItem('notes');
    if (savedNotes) {
      const parsedNotes = JSON.parse(savedNotes);
    ...
    // If no password is set, auto-unlock
    const hasPassword = localStorage.getItem('notespassword');
    if (!hasPassword) {
      setUnlocked(true);
    }
  ...
  // ── Lock: always show lock screen next time ──
  const handleLock = () => {
    // Only lock if a password is actually set
    const hasPassword = localStorage.getItem('notespassword');
    if (hasPassword) {
      setUnlocked(false);
    } else {
      // Prompt user to set a password first
      const pwd = prompt('Set a password to lock your notes:');
      if (pwd && pwd.trim() !== '') {
        localStorage.setItem('notespassword', pwd.trim());
        alert('Password set! Notes are now locked.');
        setUnlocked(false);
      }
    }
  };
  ...
  const handleColorChange = (event) => {
    setCurrentNote(prev => ({ ...prev, color: event.target.value }));
    setIsEditing(true);
  };
  ...
  const handleImageChange = (event) => {
    const files = Array.from(event.target.files);
    const newImages = files.map(file => URL.createObjectURL(file));
    setCurrentNote(prev => ({ ...prev, images: [...(prev.images || []), ...newImages] }));
    setIsEditing(true);
  };
        ...
        {activeNoteId ? (
          <div className="editor-content">
            ...
            <div className="notes-tools">
              ...
              {/* Lock button */}
              <button className="tool-btn lock-btn" title="Lock Notes" onClick={handleLock}>
                <Lock size={16} />
              </button>
            </div>
